...
#OUTFILE="barrett.csv"
OUTFILE="/mnt/pd/Rserection/process/barrett.csv"
rm ${OUTFILE}
    ...
    then
        ...
        then
            ...
            then
                ...
                # print everything
                echo "\"$stripped_ssn\",\"${shop:3:3}\",\"$last_name\",\"$first_name\",\"${gross_pay}\",4K,\"$deduction\",PS,$ps_amount,\"$loan_one\",\"$loan_two\",,\"$dob\",\"$doh\",\"$dot\",\"$address\",\"$city\",\"$state\",\"$zip\"" >> ${OUTFILE}
